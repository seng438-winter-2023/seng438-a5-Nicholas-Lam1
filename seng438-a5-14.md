...
![Laplace Trend Test](image/seng438-a5-14/Laplace%20Test.png)
...
During the demo, we noted that "Failure Report 10.docx" did not work for geometric, so we were instructed to find another dataset to demonstrate the geo metric model. The graph below shows the results from "Failure Report 2.docx".

![1680931669465](image/seng438-a5-14/1680931669465.png)
...
| Cons<br />  - Does not provide information on specific defects, so time must be taken to interpret the data<br />  - Mainly provides information on persistent failures and not intermitent failures              |
...
Distribution of work was somewhat dictated by what tools would run on which group members computer. Group members would then work on sections of the report corresponding to what tools they could run. A more specific breakdown of tasks can be seen below:

Reliability Growth Testing (Model Ranking, Laplace, Failure Intensity) - Nick

Reliability Growth Testing (Laplace, Assessment) - Shahdad

Reliability Demonstration Chart (Assessment, Plots) - Shahdad / Jaron

Comparison of results - Shahdad

Pros and Cons - Hamza

# Difficulties encountered, challenges overcome, and lessons learned

One of the learned lessons is the fact that based of input data charachteristics, either one or both methods can be used for analysis. In general, assessment using reliability growth testing is a better choice for large input data and assessment using reliability demonostration chart is a better option for limited number of inputs.

# Comments/feedback on the lab itself

One of the encountered issues, was finding the best method to run Laplace Trend Test. As a result, having more details about it in the lab instructions will be helpful.
More documentation for running the reliability tools would be appreciated. Limiting the options of programs and failure data to just one or two would be much better, as a lot of time is spent just figuring out which one works and is more effective.
